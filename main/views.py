...
def home(request):
    ...
    feate = Product.objects.filter(featured=True)
    ...
    if request.method == 'POST':
        ...
        if cart:
            ...
            if basket:
                ...
                basket.amount = basket.quantity * cart.price
                basket.save()
...
def payment(request):
    if request.method == 'POST':
        ...
        api_key = 'sk_test_92ae1e5ce0777ccc571af5ce56ff03335ca18117' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://localhost:8000/callback' #callback or thank you page
